...
"""
function rolling(fn::Function, span::S, data::V) where {S<:Signed, T<:Number, V<:AbstractVector{T}}
    n_in  = length(data)
...
function rolling(fn::Function, span::S, data::A) where {S<:Signed, N, T<:Number, A<:AbstractArray{T,N}}
...
function rolling(fn::Function, weights::V, data::V) where {T<:Number, V<:AbstractVector{T}}
    n_in  = length(data)
    span  = length(weights)
    (span > 1 && n_in >= span)  || throw(span_error(n_in, span))
...
function rolling(fn::Function, weights::V, data::A) where {N, T<:Number, A<:AbstractArray{T,N}}
    n_rows_in, n_cols  = size(data)
    span = length(weights)
    (span > 1 && n_rows_in >= span) || throw(span_error(n_rows_in, span))
...
function rolling(fn::Function, span::S, times::D, data::V) where {S<:Signed, N, T<:TimeType, D<:AbstractVector{T}, V<:AbstractVector{N}}
    n_in  = length(data)
    (span > 1 && n_in >= span) || throw(span_error(n_in, span))
...
function rolling(fn::Function, weights::V, times::D, data::V) where {N<:Number, T<:TimeType, D<:AbstractVector{T}, V<:AbstractVector{N}}
    n_in  = length(data)
    span  = length(weights)
    (span > 1 && n_in >= span)  || throw(span_error(n_in, span))
    (length(weights) == span)   || throw(weights_error(length(weights), span))
...
rolling(::Type{FILL_FIRST}, fn::Function, span::Int, data::V) where {T<:Number. V<:AbstractVector{T}} =
    rolling_fill_first(fn, span, data)
rolling(::Type{FILL_LAST}, fn::Function, span::Int, data::V) where {T<:Number. V<:AbstractVector{T}} =
    rolling_fill_last(fn, span, data)
rolling(::Type{FILL_BOTH}, fn::Function, span::Int, data::V) where {T<:Number. V<:AbstractVector{T}} =
    rolling_fill_both(fn, span, data)

rolling(::Type{FILL_FIRST}, fn::Function, span::Int, filler::T, data::V) where {T<:Number. V<:AbstractVector{T}} =
    rolling_fill_first(fn, span, filler, data)
rolling(::Type{FILL_LAST}, fn::Function, span::Int, filler::T, data::V) where {T<:Number. V<:AbstractVector{T}} =
    rolling_fill_last(fn, span, filler, data)
rolling(::Type{FILL_BOTH}, fn::Function, span::Int, filler::T, data::V) where {T<:Number. V<:AbstractVector{T}} =
    rolling_fill_both(fn, span, filler, data)

rolling(::Type{TAPER_FIRST}, fn::Function, span::Int, tapered_span::Int, data::V) where {T<:Number. V<:AbstractVector{T}} =
    rolling_taper_first(fn, span,  max(2, tapered_span), data)
rolling(::Type{TAPER_LAST}, fn::Function, span::Int, tapered_span::Int, data::V) where {T<:Number. V<:AbstractVector{T}} =
    rolling_taper_last(fn, span,  max(2, tapered_span), data)
rolling(::Type{TAPER_BOTH}, fn::Function, span::Int, tapered_span::Int, data::V) where {T<:Number. V<:AbstractVector{T}} =
...
"""
function rolling_fill_first(fn::Function, span::Int, data::V) where {T<:Number. V<:AbstractVector{T}}
    n_in  = length(data)
    (span > 1 && n_in >= span) || throw(span_error(n_in, span))
...
`length(result) == length(data)`
"""
function rolling_fill_first(fn::Function, span::Int, filler::T, data::V) where {T<:Number. V<:AbstractVector{T}}
    n_in  = length(data)
    (span > 1 && n_in >= span) || throw(span_error(n_in, span))
...
`length(result) == length(data)`
"""
function rolling_fill_last(fn::Function, span::Int, data::V) where {T<:Number. V<:AbstractVector{T}}
    n_in  = length(data)
    (span > 1 && n_in >= span) || throw(span_error(n_in, span))
...
`length(result) == length(data)`
"""
function rolling_fill_last(fn::Function, span::Int, filler::T, data::V) where {T<:Number. V<:AbstractVector{T}}
    n_in  = length(data)
    (span > 1 && n_in >= span) || throw(span_error(n_in, span))
...
`length(result) == length(data)`
"""
function rolling_fill_both(fn::Function, span::Int, data::V, alpha::Float64=0.5) where {T<:Number. V<:AbstractVector{T}}
...
`length(result) == length(data)`
"""
function rolling_fill_both(fn::Function, span::Int, filler::T, data::V, alpha::Float64=9.5) where {T<:Number. V<:AbstractVector{T}}
...
`length(result) == length(data)`
"""
function rolling_taper_first(fn::Function, span::Int, tapered_span::Int, data::V) where {T<:Number. V<:AbstractVector{T}}
    n_in  = length(data)
    (span > 1 && n_in >= span) || throw(span_error(n_in, span))
...
`length(result) == length(data)`
"""
function rolling_taper_last(fn::Function, span::Int, tapered_span::Int, data::V) where {T<:Number. V<:AbstractVector{T}}
    n_in  = length(data)
    (span > 1 && n_in >= span) || throw(span_error(n_in, span))
    (span > tapered_span) || throw(taperedspan_error(span, tapered_span))
...
`length(result) == length(data)`
"""
function rolling_taper_both(fn::Function, span::Int, tapered_span::Int, data::V, alpha::Float64=0.5) where {T<:Number. V<:AbstractVector{T}}
