...
function basic_rolling(arg1::D1, arg2::D2, window_span::Int, window_fn::F) where {T1,T2, D1<:AbstractVector{T1}, D2<:AbstractVector{T2}, F<:Function}
    # there are 1 or more columns, each holds `n` values
    nvalues = nrows(arg1)
    nvalues == nrows(arg2) || error()
    result_type = rts(window_fn, (T1,T2))
    ...
    results = Vector{result_type}(undef, window_covered_values)

    ilow, ihigh = 1, window_span

    @inbounds for idx in eachindex(results)
        results[idx] = window_fn(arg1[ilow:ihigh], arg2[ilow:ihigh])
        ilow = ilow + 1
        ihigh = ihigh + 1
    end
...
function basic_rolling(data::D, window_span::Int, window_fn::Function) where {T,D<:AbstractMatrix{T}}
    ...
    nvalues = nrows(data)
    nargs = ncols(data)
    result_type = rts(window_fn, map(typeof,data[1,:]))
    ...
    results = Matrix{result_type}(undef, window_covered_values, length(result_type))
