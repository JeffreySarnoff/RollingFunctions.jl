...
function pidfinal_tiling(fn::F, width::Integer,
    data::AbstractMatrix{T}, weighting::AbstractWeights{T}, padding) where {T,F<:Function}
    colcount = ncols(data)
    mweights = vmatrix(weighting, colcount)

    pidfinal_tiling(fn, width, data, mweights, padding)
end
...
function pidfinal_tiling(fn::F, width::Integer,
    ...
    pidfinal_tiling(fn, width, data, mweights, paddomg)
end

@inline function pidfinal_tiling(fn::F, width::Integer,
    data::AbstractMatrix{T},weighting::AbstractMatrix{T}, padding) where {T,F<:Function}
    ᵛʷdata = asview(data)
    ᵛʷweights = asview(weighting)

    pidfinal_tiling(fn, width, ᵛʷdata, ᵛʷweights, padding)
end

function pidfinal_tiling(fn::F, width::Integer,
    data::AbstractMatrix{T}, weighting::AbstractWeights{W}, padding) where {T,W,F<:Function}
    if T <: Integer
        return pidfinal_tiling(fn, width, Matrix{W}(data), weighting, padding)
    end
...
@inline function pidfinal_tiling(fn::F, width::Integer,
    ...
    pidfinal_tiling(fn, width, data, Matrix{T}(weighting), padding)
end
