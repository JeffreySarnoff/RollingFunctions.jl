...
@test tiling(F, W, D₁, D₂, weights) == expected
...
@test tiling(F, M, W) == expected
@test typeof(tiling(F, M, W)) == typeof(expected)
...
@test map(clean, tiling(F, M, W; padding=missing)) == map(clean, expected)
@test typeof(tiling(F, M, W; padding=missing)) == typeof(expected)
...
@test map(clean, tiling(F, M, W; padding=missing, padlast=true)) == map(clean, expected)
@test typeof(tiling(F, M, W; padding=missing, padlast=true)) == typeof(expected)
=#
