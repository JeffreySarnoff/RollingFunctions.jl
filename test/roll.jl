...
result = rolling(intvec, window_span, window_fn; padding = missing, padlast=true)
expected = [missing, missing, missing, missing, missing, missing, 28, 35, 42, 49]
@test map(clean, result) == map(clean, expected)
@test typeof(result) == typeof(expected)

result = rolling(floatvec, window_span, window_fn; padding = missing, padlast=true)
expected = [28.0, 35.0, 42.0, 49.0]
@test map(clean, result) == map(clean, expected)
@test typeof(result) == typeof(expected)

result = rolling(intmat, window_span, window_fn; padding = missing, padlast=true)
expected = [28 49; 35 42; 42 35; 49 28]
@test map(clean, result) == map(clean, expected)
@test typeof(result) == typeof(expected)

result = rolling(floatmat, window_span, window_fn; padding = missing, padlast=true)
expected = [28.0 49.0; 35.0 42.0; 42.0 35.0; 49.0 28.0]
@test map(clean, result) == map(clean, expected)
@test typeof(result) == typeof(expected)
